...
Pela [Figura 2](#Protocolos) é possível perceber que entre o Broker e os Dispositivos são utilizados os protocolos TCP e UDP. Enquanto o primeiro é utilizado para o envio e a confirmação de comandos, já que ele é uma abordagem confiável de envio de mensagens, o segundo é usado para o envio de dados, e embora ele não garanta a confiabilidade do envio, ele permite que os dados sejam enviados de maneira mais dinâmica, sem que seja necessária a confirmação e permitindo que o envio de dados seja mais frequente em um curto espaço de tempo.
...
A Aplicação é o componente no qual é possível ver a temperatura de cada um dos sensores, acompanhar o status deles e fazer a gestão desse status. A [Figura 3](#InterfaceApp) mostra a interface gráfica com as informações acerca dos dispositivos, sobre a conexão da Aplicação com o Broker e o botão que possibilita Ligar/Desligar o dispositivo selecionado. Como já dito a Aplicação faz solicitações HTTP para o Broker com o objetivo de conseguir as informações dos dispositivos bem como editar o status deles, a [Figura 2](#Protocolos) também traz as rotas e os verbos usados nas requisições feitas pela Aplicação, assim como a resposta do Broker para cada uma delas.
